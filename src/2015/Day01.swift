...
@main final class Today: Day {
    init() { super.init(day: 1, year: 2015) }
    /*
    ...
    func determineFinalLocation(following directions: String) -> Int {
        var currentFloor = 0
        for step in directions {
            if step == "(" { currentFloor += 1 }
            else if step == ")" { currentFloor -= 1 }
        }
        return currentFloor
    }
    ...
    func stepsToReachBasement(following directions: String) -> Int {
        var currentFloor = 0
    ...
    override func run() {
        ...
        print("Santa ends up on floor \(determineFinalLocation(following: self.data))")
        print("The first time Santa enters the basement is at step \(stepsToReachBasement(following: self.data))")
    }
