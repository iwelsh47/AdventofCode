...
@main final class Today: Day {
    init() { super.init(day: 2, year: 2015) }
    /*
    ...
    func getDimensions(from dimensions: String) -> (Int, Int, Int) {
        return dimensions.components(separatedBy: "x").map{ Int($0)! }.splat()
    }
    
    func calculateRequiredPaper(forBoxDimensions dimensions: String) -> Int {
        let (length, width, height) = getDimensions(from: dimensions)
    ...
    func calculateRequiredRibbon(forBoxDimensions dimensions: String) -> Int {
        let (length, width, height) = getDimensions(from: dimensions)
    ...
    override func run() {
        ...
        let totalPaperArea = data
            .components(separatedBy: "\n")
            .reduce(0, { x, dimensions -> Int in x + calculateRequiredPaper(forBoxDimensions: dimensions) })
        print("Total square footage of paper to order is \(totalPaperArea) square feet")
        
        let totalRibbonLength = data
            .components(separatedBy: "\n")
            .reduce(0, { x, dimensions -> Int in x + calculateRequiredRibbon(forBoxDimensions: dimensions) })
        print("Total ribbon length required is \(totalRibbonLength) feet.")
    }
